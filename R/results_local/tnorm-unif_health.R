...
filename <- 'tnorm-unif_health_all_rdmh'
ptitle <- 'truncated-normal--uniform model, Healthy all (RDMH)'
datafile <- 'data_H.dat'
...
data <- matrix(0, N, d)
data[,] <- t(read.matrix(datafile))[1:N,1:d]
colnames(data) <- y.names
rownames(data) <- sprintf("id[%d]",1:N)
d1 <- d+1
d2 <- 2*d
d3 <- 2*d+1
...
## }
PGF <- function(data){
    mu <- interval(rnorm(d,dmean,sd(dmean)),0,1)
...
for(i in 1:nsamples){
    ...
    ##
    covc <- cho %*% diag(exp(sample0$Posterior1[i,pls])/sdU)
    samples[i,1:d] <- rtmvnorm(1, mean=sample0$Posterior1[i,pmu],  sigma=t(covc) %*% covc, lower=lowerl, upper=upperl)
}
