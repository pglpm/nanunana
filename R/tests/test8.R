...
n0 <- 4
mean0 <- c(0,0)
sigma0 <- matrix(c(10000,0,0,10000),2)
...
PGF <- function(data){
    mu <- rmvnorm(1,mean=mean0,sigma=sigma0)
    lvar1 <- rnorm(1,0,log(1e6))
    lvar2 <- rnorm(1,0,log(1e6))
    rho <- runif(1,-1,1)
    return(c(mu,lvar1,lvar2,rho))
}
...
hyperprior0 <- function(parm,data){
    ...
    mu.prior <- dmvnorm(mu,mean=mean0,sigma=sigma0, log=T)
    ## prior for variances is Jeffreys = uniform in log
    lvar1.prior <- dnorm(parm[3],0,log(1e6),log=T)
    lvar2.prior <- dnorm(parm[4],0,log(1e6),log=T)
    ...
    LL <- sum(dmvnorm(data$data,mean=mu,sigma=varm, log=T))
    LP <- LL + mu.prior + lvar1.prior + lvar2.prior + rho.prior
