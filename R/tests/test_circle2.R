...
np <- d  # total num. parameters

## training data with means and scatter matrix
nt <- 500 # number of traninig data
datat <- matrix(0,nt,d)
sigmat <- diag(d)*0.01
angle <- seq(0,2*pi,length.out=nt)
for(i in 1:nt){
datat[i,] <- rmvnorm(1,c(1+cos(angle[i]),1+sin(angle[i])),sigmat)
}
...
datanew <- signif(datat[1,],2)
truepnew <- 0 # exact probability of extra datum


## parameters: mean, log-variances, logit-correlations, all unbounded
parm.names <- as.parm.names(list(mu=rep(0,d)))
...
#    print(sum(c(covm-(stds %*% rho %*% stds))^2)) # check
    ## log-likelihood
    LL <- if(sum((data$data-mu)^2)<sigmat^2){sum(dmvnorm(data$data, mu, sigmat, log=T))} else {-1e16}
#    LL <- sum(dmvnorm(data$data, mu, sigmat, log=T))
    ## log-posterior
    LP <- LL + mu.prior 
...
for(j in 1:(np-1)){
    for(i in (j+1):np){
        dat <- data.frame(x=sample0$Posterior2[,j],y= sample0$Posterior2[,i])
        print(
            ggplot() + stat_bin2d(data=dat,aes(x=x,y=y),binwidth=0.0001) +
            scale_fill_gradientn(colours=brewer.pal(n=9,name='Blues')) +
            labs(x=parm.names[j],y=parm.names[i]) +
            geom_point(data=data.frame(p1=sample0$Summary2[j,'Mean'], p2=sample0$Summary2[i,'Mean']),aes(p1,p2),colour=myred,size=4)
            )
    }}
dev.off()
...
pdf('justatest.pdf')
       dat <- data.frame(x=datat[,1],y=datat[,2])
        print(
            ggplot() + stat_bin2d(data=dat,aes(x=x,y=y),binwidth=0.1) +
            scale_fill_gradientn(colours=brewer.pal(n=9,name='Blues'))
            )
dev.off()
...
rho <- 0.9
std <- c(1.2,1)
sigm <- diag(std) %*% matrix(c(1,rho,rho,1),2,2) %*% diag(std)
datat <- invlogit(rmvnorm(1e6,c(1.1,-1),sigm))*2-1
pdf('justatest.pdf')
       dat <- data.frame(x=datat[,1],y=datat[,2])
        print(
            ggplot() + stat_bin2d(data=dat,aes(x=x,y=y),binwidth=0.02) +
            scale_fill_gradientn(colours=brewer.pal(n=9,name='Blues')) +
            xlim(-1,1) + ylim(-1,1)
            )
dev.off()
