...
HfromH <- foreach(s = 1:numH, .combine=c)%do%{
    unname(unlist(read.csv(paste0('logscore_', groups$H, '_marg_', s, '.csv'),header=FALSE,sep=',')))
}

HfromS <- unname(unlist(read.csv(paste0('logscore_', groups$S, '_marg_', numS+1, '.csv'),header=FALSE,sep=',')))

looH <- cbind(HfromH - HfromS, HfromH, HfromS)

write.table(looH, file= paste0('cvls_', groups$H,'.csv'), row.names=FALSE, col.names=FALSE, sep=',')


SfromS <- foreach(s = 1:numS, .combine=c)%do%{
    unname(unlist(read.csv(paste0('logscore_', groups$S, '_marg_', s, '.csv'),header=FALSE,sep=',')))
}

SfromH <- unname(unlist(read.csv(paste0('logscore_', groups$H, '_marg_', numH+1, '.csv'),header=FALSE,sep=',')))

looS <- cbind(SfromS - SfromH, SfromS, SfromH)
...
savenames <- paste0('fits_', groups, '_marg_')
