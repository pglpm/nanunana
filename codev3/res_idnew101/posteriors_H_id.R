...
dp <- DirichletProcessMvnormal(x,
                               alphaPriors=c(1, 0.1),
                               g0Priors=list(mu0=rep(0,dims), kappa0=meansd/sdmu, Lambda=diag(dims)/(sqrt(2)*meansd^4/sdsd^2+2*meansd^2), nu=dims-1+(sqrt(2)*meansd^2/sdsd^2+4)), numInitialClusters = 2)
