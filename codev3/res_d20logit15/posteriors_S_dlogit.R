...
#foreach(s = 1:(datasize+1))%dopar%{
    ...
    dp <- DirichletProcessMvnormal(x,
                               alphaPriors=c(1^2/5^2, 1/5^2),
                               g0Priors=list(mu0=rep(0,dims), kappa0=1, Lambda=diag(dims)/(dims+2), nu=dims+1), numInitialClusters = 2)
...
fitdp <- myfitfast(fitdp,its=2000,progressBar=FALSE)
