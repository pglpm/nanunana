...
for groupid, group in enumerate(groups):
    ...
    for ii in group:
        ...
        if os.path.isfile(path+ii+directory+prop2):
             ...
             graphweights_all += [stats.moment(graphweights, moment=4)]
             ## shortest path
             sp = numpy.load(path+ii+directory+prop2)
             sp=sp.flatten()
             sp_all += [stats.moment(sp, moment=4)]
             ## cluster coefficient
             cc = numpy.load(path+ii+directory+prop3)
             cc = cc.flatten()
             cc_all += [stats.moment(cc, moment=4)]
             ## weighted degree
             wd = numpy.load(path+ii+directory+prop4)
             wd=wd.flatten()
             wd_all += [stats.moment(wd, moment=4)]
             ## modularity
             ...
             wcc_all += [stats.moment(gw_avg, moment=4)]   
             ...
             nwd_all += [stats.moment(nwd, moment=4)]   
             ## number of nodes
             ns_all += [len(wd)]
        else:
...
#        os.system('ipython properties_mean.py ' + en+ ' '+str(fi))
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('4th moment averaged edge weights') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('4th moment averaged shortest path') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('4th moment averaged cluster coefficient') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('4th moment averaged weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('4th moment weighted closeness centrality')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('4th moment normalized weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlabel('4th moment edge weights') 
    pylab.ylabel('4th moment shortest path') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_sp%s_kurt.eps' %(directory,ending) )
    

    pylab.figure('compare gw-cc')
    pylab.plot(graphweights_all,cc_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('4th moment cluster coefficient') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_cc%s_kurt.eps' %(directory,ending) )

    pylab.figure('compare gw-wd')
    pylab.plot(graphweights_all,wd_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('4th moment weighted degree')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_wd%s_kurt.eps' %(directory,ending) )
    
    pylab.figure('compare gw-mod')
    pylab.plot(graphweights_all,mod_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('modularity')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_mod_kurt%s.eps' %(directory,ending) )

    pylab.figure('compare gw-ns')
    pylab.plot(graphweights_all,ns_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('number of nodes') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_ns%s_kurt.eps' %(directory,ending) )

    pylab.figure('compare gw-wcc')
    pylab.plot(graphweights_all,wcc_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('4th moment weighted closeness centrality')    
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_wcc_kurt%s.eps' %(directory,ending) )

    pylab.figure('compare gw-nwd')
    pylab.plot(graphweights_all,nwd_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('4th moment edge weights')
    pylab.ylabel('4th moment normalized weighted degree')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_nwd_kurt%s.eps' %(directory,ending) )
    '''
...
if not os.path.isdir('/home/bachmann/AD_fMRI/results/%s/' %directory):
    os.mkdir('/home/bachmann/AD_fMRI/results/%s/' %directory)
pylab.savefig('/home/bachmann/AD_fMRI/results/%s/result_overview%s_kurt.eps' %(directory,ending) )
filx=[22,16]
