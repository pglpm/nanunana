...
for groupid, group in enumerate(groups):
    ...
    for ii in group:
        ...
        if os.path.isfile(path+ii+directory+prop2):
             ## graph weights
             graphweights = 1./numpy.load(path+ii+directory+prop)
             graphweights_all += [numpy.mean(graphweights.flatten())]
             ## shortest path
             sp = numpy.load(path+ii+directory+prop2)
             sp=sp.flatten()
             sp_all += [numpy.mean(sp)]
             ## cluster coefficient
             cc = numpy.load(path+ii+directory+prop3)
             cc = cc.flatten()
             cc_all += [numpy.mean(cc)]
             ## weighted degree
             wd = numpy.load(path+ii+directory+prop4)
             wd=wd.flatten()
             wd_all += [numpy.mean(wd)] 
             ## modularity
             ...
             nwd_all += [numpy.mean(nwd)]   
             ## number of nodes
             ns_all += [len(wd)]
        else:
	     print 'data %s does not exist' %path+ii+directory+prop4
    print group_names[groupid]
    print ' graphweights:', ', '.join(map(str, list(numpy.round(graphweights_all,6).astype(str)))) 
    print ' shortest path:',', '.join(map(str, list(numpy.round(sp_all,6).astype(str)))) 
    print ' cluster coeff.:',', '.join(map(str, list(numpy.round(cc_all,6).astype(str)))) 
    print ' weighted degree:',', '.join(map(str, list(numpy.round(wd_all,6).astype(str)))) 
    print ' normalized weighted degree',', '.join(map(str, list(numpy.round(nwd_all,6).astype(str)))) 
    print ' node size:',', '.join(map(str, list(numpy.round(ns_all,6).astype(str)))) 
    print ' closeness centrality:',', '.join(map(str, list(numpy.round(wcc_all,6).astype(str)))) 
    print ' modularity:',', '.join(map(str, list(numpy.round(mod_all,6).astype(str)))) 
...
#        os.system('ipython properties_mean.py ' + en+ ' '+str(fi))
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('averaged edge weights') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('averaged shortest path') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('averaged cluster coefficient') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('averaged weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('weighted closeness centrality')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('normalized weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlabel('averaged edge weights') 
    pylab.ylabel('averaged shortest path') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_sp%s.eps' %(directory,ending) )
...
if not os.path.isdir('/home/bachmann/AD_fMRI/results/%s/' %directory):
    os.mkdir('/home/bachmann/AD_fMRI/results/%s/' %directory)
pylab.savefig('/home/bachmann/AD_fMRI/results/%s/result_overview%s.eps' %(directory,ending) )
filx=[22,16]
