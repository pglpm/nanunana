...
for groupid, group in enumerate(groups):
    ...
    for ii in group:
        ...
        if os.path.isfile(path+ii+directory+prop2):
             ...
             graphweights_all += [stats.moment(graphweights, moment=2)]
             print ii, stats.moment(graphweights, moment=2)
             ## shortest path
             sp = numpy.load(path+ii+directory+prop2)
             sp=sp.flatten()
             sp_all += [stats.moment(sp, moment=2)]
             ## cluster coefficient
             cc = numpy.load(path+ii+directory+prop3)
             cc = cc.flatten()
             cc_all += [stats.moment(cc, moment=2)]
             ## weighted degree
             wd = numpy.load(path+ii+directory+prop4)
             wd=wd.flatten()
             wd_all += [stats.moment(wd, moment=2)]
             ## modularity
             ...
             wcc_all += [stats.moment(gw_avg, moment=2)]   
             ...
             nwd_all += [stats.moment(nwd, moment=2)]   
             ## number of nodes
             ns_all += [len(wd)]
        else:
...
#        os.system('ipython properties_mean.py ' + en+ ' '+str(fi))
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('var moment averaged edge weights') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))   
    pylab.ylabel('var moment averaged shortest path') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('var moment averaged cluster coefficient') 
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('var moment averaged weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('var moment weighted closeness centrality')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlim((-1,3))
    pylab.ylabel('var moment normalized weighted degree')      
    pylab.xticks([0.,1.,2.], ['C','MCI','AD'])
    ...
    pylab.xlabel('var moment edge weights') 
    pylab.ylabel('var moment shortest path') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_sp%s_var.eps' %(directory,ending) )
    

    pylab.figure('compare gw-cc')
    pylab.plot(graphweights_all,cc_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('var moment cluster coefficient') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_cc%s_var.eps' %(directory,ending) )

    pylab.figure('compare gw-wd')
    pylab.plot(graphweights_all,wd_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('var moment weighted degree')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_wd%s_var.eps' %(directory,ending) )
    
    pylab.figure('compare gw-mod')
    pylab.plot(graphweights_all,mod_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('modularity')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_mod_var%s.eps' %(directory,ending) )

    pylab.figure('compare gw-ns')
    pylab.plot(graphweights_all,ns_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('number of nodes') 
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_ns_var%s.eps' %(directory,ending) )

    pylab.figure('compare gw-wcc')
    pylab.plot(graphweights_all,wcc_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('var moment weighted closeness centrality')    
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_wcc_var%s.eps' %(directory,ending) )

    pylab.figure('compare gw-nwd')
    pylab.plot(graphweights_all,nwd_all,'.', label=group_names[groupid],color=colors[groupid])
    pylab.xlabel('var moment edge weights')
    pylab.ylabel('var moment normalized weighted degree')
    pylab.savefig('/home/bachmann/AD_fMRI/results/%s/compare_gw_nwd_var%s.eps' %(directory,ending) )
    '''
...
if not os.path.isdir('/home/bachmann/AD_fMRI/results/%s/' %directory):
    os.mkdir('/home/bachmann/AD_fMRI/results/%s/' %directory)
pylab.savefig('/home/bachmann/AD_fMRI/results/%s/result_overview%s_var.eps' %(directory,ending) )
filx=[22,16]
